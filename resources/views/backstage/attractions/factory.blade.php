...
<div class="container-fluid">
  ...
  <div class="card shadow mb-4">
    ...
    <div class="mb-3">
      ...
      <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name', optional($attraction ?? null)->name) }}" />
    </div>
    <div class="mb-3">
      <label for="description" class="form-label">簡介 Description</label>
      <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="3">{{ old('description', optional($attraction ?? null)->description) }}</textarea>
    </div>
    <div class="row mb-3">
      <div class="col">
        <label for="website" class="form-label">官方網站 Website</label>
        <input type="text" class="form-control @error('website') is-invalid @enderror" id="website" name="website" value="{{ old('website', optional($attraction ?? null)->website) }}" />
      </div>
      <div class="col">
        <label for="tel" class="form-label">聯絡電話 Tel</label>
        <input type="text" class="form-control @error('tel') is-invalid @enderror" id="tel" name="tel" value="{{ old('tel', optional($attraction ?? null)->tel) }}" />
      </div>
    </div>
    <div class="row mb-3">
      <div class="col">
        <label for="ticket_info" class="form-label">售票資訊 Ticket Info</label>
        <input type="text" class="form-control @error('ticket_info') is-invalid @enderror" id="ticket_info" name="ticket_info" value="{{ old('ticket_info', optional($attraction ?? null)->ticket_info) }}" />
      </div>
      <div class="col">
        <label for="traffic_info" class="form-label">交通資訊 Traffic Info</label>
        <input type="text" class="form-control @error('traffic_info') is-invalid @enderror" id="traffic_info" name="traffic_info" value="{{ old('traffic_info', optional($attraction ?? null)->traffic_info) }}" />
      </div>
      <div class="col">
        <label for="parking_info" class="form-label">停車資訊 Parking Info</label>
        <input type="text" class="form-control @error('parking_info') is-invalid @enderror" id="parking_info" name="parking_info" value="{{ old('parking_info', optional($attraction ?? null)->parking_info) }}" />
      </div>
    </div>
    <hr />
    {{-- --}}
